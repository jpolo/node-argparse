 ...
 * Utils methods
 */
var $dump = function(object, depth, references) {
	depth = depth || 2;
		...
		case 'object':
			...
			if(object instanceof Array) {
				return '[' + object.map(function(value) {
					return $dump(value, depth, referencesNew);
				}).join(', ')+ ']';
			} else {
				var className = $value(object.constructor.__name__, 'Object');
				...
				if(depth > 1) {
					...
					for(var property in object) {
						...
						if((value != undefined) && !(value instanceof Function)) {
							valueString = $dump(value, depth - 1, referencesNew);
						}
...
/**
 * Formatter for generating usage messages and argument help strings. Only the name of this class is considered a public API. All the methods provided
 * by the class are considered an implementation detail.
 */
function HelpFormatter(options) {
	...
	this.sectionRoot = new _HelpSection(this);
...
HelpFormatter.prototype.startSection = function(/* string */ heading) {
	this._indent();
	this.sectionCurrent = new _HelpSection(this, this.sectionCurrent, heading);
...
HelpFormatter.prototype._expandHelp = function(action) {
    ...
    /*
	 * for name in list(params): if hasattr(params[name], '__name__'): params[name] = params[name].__name__
	 */
 ...
 */
_HelpSection = function(formatter, parent, heading) {
...
_HelpSection.prototype.addItem = function(callback) {
	this.items.push(callback);
	return this;
};

_HelpSection.prototype.formatHelp = function() {
...
/**
 * Help message formatter which retains any formatting in descriptions. Only the name of this class is considered a public API. All the methods
...
sys.inherits(HelpFormatterRawDescription, HelpFormatter);
...
/**
 * Help message formatter which retains formatting of all help text. Only the name of this class is considered a public API. All the methods provided
 * by the class are considered an implementation detail.
 */
function HelpFormatterRawText(options) {
	HelpFormatterRawDescription.call(this, options);
};
sys.inherits(HelpFormatterRawText, HelpFormatterRawDescription);
...
/**
 * Help message formatter which adds default values to argument help. Only the name of this class is considered a public API. All the methods provided
 * by the class are considered an implementation detail.
 */
function HelpFormatterArgumentDefaults(options) {
	HelpFormatter.call(this, options);
};
sys.inherits(HelpFormatterArgumentDefaults, HelpFormatter);
 ...
 */
/**
 * Information about how to convert command line strings to Javascript objects. Action objects are used by an ArgumentParser to represent the
 * information needed to parse a single argument from one or more strings from the command line. The keyword arguments to the Action constructor are
 * also all attributes of Action instances. Keyword Arguments: - optionStrings -- A list of command-line option strings which should be associated
 * with this action. - destination -- The name of the attribute to hold the created object(s) - nargs -- The number of command-line arguments that
 * should be consumed. By default, one argument will be consumed and a single value will be produced. Other values include: - N (an integer) consumes
 * N arguments (and produces a list) - '?' consumes zero or one arguments - '*' consumes zero or more arguments (and produces a list) - '+' consumes
 * one or more arguments (and produces a list) Note that the difference between the default and nargs=1 is that with the default, a single value will
 * be produced, while with nargs=1, a list containing a single value will be produced. - constant -- The value to be produced if the option is
 * specified and the option uses an action that takes no values. - defaultValue -- The value to be produced if the option is not specified. - type --
 * The type which the command-line arguments should be converted to, should be one of 'string', 'int', 'float', 'complex' or a callable object that
 * accepts a single string argument. If None, 'string' is assumed. - choices -- A container of values that should be allowed. If not None, after a
 * command-line argument has been converted to the appropriate type, an exception will be raised if it is not a member of this collection. - required --
 * True if the action must always be specified at the command line. This is only meaningful for optional command-line arguments. - help -- The help
 * string describing the argument. - metavar -- The name to be used for the option's argument with the help string. If None, the 'destination' value
...
sys.inherits(ActionStore, Action);
...
sys.inherits(ActionStoreConstant, Action);
...
sys.inherits(ActionStoreTrue, ActionStoreConstant);
...
sys.inherits(ActionStoreFalse, ActionStoreConstant);
...
sys.inherits(ActionAppend, Action);

ActionAppend.prototype.call = function(parser, namespace, values, optionString) {
	items = [].concat($value(namespace[this.destination], []), values);
...
sys.inherits(ActionAppendConstant, Action);

ActionAppendConstant.prototype.call = function(parser, namespace, values, optionString) {
	items = [].concat($value(namespace[this.destination], []));
...
sys.inherits(ActionCount, Action);
...
sys.inherits(ActionHelp, Action);
...
sys.inherits(ActionVersion, Action);
...
sys.inherits(ActionSubparser, Action);
...
ActionSubparser.prototype.call = function(parser, namespace, values, optionString) {
	 ...
	 * 
	 * def __init__(this, name, help): sup = super(ActionSubparser._ChoicesPseudoAction, this) sup.__init__(optionStrings=[], dest=name, help=help)
	 */
...
Namespace.prototype.set = function(property, value) {
    this[property] = value;
    return this;
};

Namespace.prototype.get = function(property) {
 ...
 */
/**
 * addArgument(dest, ..., name=value, ...) addArgument(optionString, optionString, ..., name=value, ...)
 */
...
ActionContainer.prototype.addArgumentGroup = function(options, mutuallyExclusive) {
	...
	if(mutuallyExclusive) {
		var group = new ArgumentGroupMutex(this, options);
...
ActionContainer.prototype._handleConflictError = function(action, conflictingActions) {
    ...
    var message = $stringPrint(_('Conflicting option string(s) : %conflict%'), {conflict: optionStrings.join(', ')});
...
sys.inherits(ArgumentGroup, ActionContainer);
...
sys.inherits(ArgumentGroupMutex, ArgumentGroup);
...
function ArgumentParser(options) {
	...
	// environment
	options.debug = $value(options.debug, false);
	options.stdout = $value(options.stdout, process.stdout);
	options.stderr = $value(options.stderr, process.stderr);
	
	// default program name
	options.program = $value(options.program, require('path').basename(process.execPath));
...
sys.inherits(ArgumentParser, ActionContainer);
...
ArgumentParser.prototype.addSubparsers =  function(options) {
    ...
    options.parserClass =  $value(options.parserClass, 'ArgumentParser');
    

    if($defined(options.title) || $defined(options.description)) {

        this._subparsers = this.addArgumentGroup({
            title: _($value(options.title, 'subcommands')), 
            description: _($value(options.description))
        });
...
ArgumentParser.prototype.parseArgs = function(/* array */ args, /* object */ namespace) {
    ...
    if (argv && argv.length > 0) {
        this.error(1, $stringPrint(_('Unrecognized arguments: %arguments%.'), {arguments: argv.join(' ')}));
    }
    return args;
};
...
ArgumentParser.prototype._getOptionTuples = function(optionString) {
	 ...
	 if(chars.indexOf(optionString[0]) >= 0 && chars.indexOf(optionString[1]) >= 0) {
		 ...
		 for(var optionString in this._optionStringActions) {
...
ArgumentParser.prototype._getValues = function(action, argStrings) {
    ...
    // all other types of nargs produce a list
    	...
    	value = argStrings.map(function(v) {return this._getValue(action, v);}.bind(this));
...
ArgumentParser.prototype._getValue = function(action, argString) {
    ...
    if(!$isCallable(typeFunc)) {
	var message = $stringPrint(_('%callback% is not callable'), {callback: typeFunc});
        throw new ArgumentError(action, message);
    }
    var result;
    ...
    // ArgumentTypeErrors indicate errors
  		...
  		// catch ArgumentTypeError:
            /*
			 * name = action.type; message = e.message;//TODO change this throw new ArgumentError(action, message);
			 */

        // TypeErrors or ValueErrors also indicate errors
        // catch (TypeError, ValueError):
  	    var message = $stringPrint(_('Invalid %name% value: %value%'), {name: action.type, value: argString});
            throw new ArgumentError(action, message);
  	}
    // return the converted value
    return result;
...
/**
 * Error method Prints a usage message incorporating the message to stderr and exits. If you override this in a subclass, it should not return -- it
...
/**
 * An error from creating or using an argument (optional or positional). The string value of this exception is the message, augmented with information
...
sys.inherits(ArgumentError, Error);
