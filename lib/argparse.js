...
var _ActionsContainer = Class.create({
	initialize: function(options) {
	    ...
	    this.conflictHandler = $value(options['conflictHandler']);
	...
	addArgument: function(args, kwargs) {//TODO change python syntax
	    ...
	    if(!args || args.length == 1 && chars.indexOf(args[0][0]) < 0) {
        ...
        if(! _callable(actionClass)) {
            throw ValueError('unknown action "' + actionClass + '"' % actionClass);
        }
        action = actionClass(kwargs);

        //raise an error if the action type is not callable
        typeFunc = this._registryGet('type', action.type, action.type);
        if(! _callable(typeFunc)) {
            throw ValueError(typeFunc + ' is not callable');
        }
	...
	_getOptionalKwargs: function(args, kwargs) {
        ...
        args.forEach(function(optionString) {
            ...
            if(this.prefixChars.indexOf(optionString[0]) < 0) {
            	throw ValueError(_('invalid option string '+optionString+': must start with a character ' + this.prefixChars));
            }

            // strings starting with two prefix characters are long options
            optionStrings.push(optionString);
            if(this.prefixChars.indexOf(optionString[0]) >= 0) {
                if(optionString.length > 1 && this.prefixChars.indexOf(optionString[1]) >= 0) {
        ...
        if(!$defined(destination)) {
            ...
            if(!destination) {
                throw ValueError(_('destination= is required for options like ' + optionString));
            }
	...
	_getHandler: function() {
        ...
        handlerFuncName = '_handle_conflict_' + this.conflictHandler;
        if(!$defined(this[handlerFuncName])) {
             throw ValueError(_('invalid conflictResolution value: ' + this.conflictHandler));
        }
	...
	_checkConflict: function(action) {
        ...
        conflictOptionals = [];
        action.optionStrings.forEach(function(optionString) {
        	if(this._optionStringActions.indexOf(optionString) >= 0) {
            	conflictOptional = this._optionStringActions[optionString];
            	conflictOptionals.push([optionString, conflictOptional]);
        	}
        });

        // resolve any conflicts
        if(conflictOptionals) {
...
var _ArgumentGroup = _ActionsContainer.extend({
	...
	_addAction: function(action) {
        action = super(_ArgumentGroup, self)._addAction(action);
        this._groupActions.push(action);
        return action;
	},
	_removeAction: function(action) {
        super(_ArgumentGroup, self)._removeAction(action);
        this._groupActions.remove(action);
	}
});

var _MutuallyExclusiveGroup = _ArgumentGroup.extend({
	...
	_addAction: function(action) {
		if(action.required) {
            throw ValueError(_('mutually exclusive arguments must be optional'));
		}
	...
	_removeAction: function(action) {
        ...
        this._groupActions.remove(action);//Change this
	}
});
