...
var _ActionContainer = Class.create('_ActionContainer', {
	...
	addArgument: function(args, kwargs) {//TODO change python syntax
	    ...
	    if(!args || args.length == 1 && this.prefixChars.indexOf(args[0][0]) < 0) {
            if(args && $defined(kwargs['destination'])) {
                throw ValueError('destination supplied twice for positional argument');
            }
        ...
        if(! $isCallable(actionClass)) {
            throw 'unknown action "' + actionClass + '"';//TODO : ValueError
        }
        ...
        if( $isCallable(typeFunc)) {
            throw typeFunc + ' is not callable';//TODO : ValueError
        }
	 ...
	 _addContainerActions: function(container) {
        ...
        this._actionGroups.forEach(function(group) {
        	if(titleGroupMap.indexOf(group.title) >= 0) {
                throw ValueError(_('cannot merge actions - two groups are named ') + group.title);
        	}
	...
	_getPositionalKwargs: function(destination, kwargs) {
        ...
        if($defined(kwargs['required'])) {
            throw TypeError(_("'required' is an invalid argument for positionals"));
        }
	...
	_getOptionalKwargs: function(args, kwargs) {
        ...
        args.forEach(function(optionString) {
            ...
            if(prefixChars.indexOf(optionString[0]) < 0) {
            	throw ValueError(_('invalid option string ' + optionString + ': must start with a character ' + prefixChars));//TODO Change this
            }
        ...
        if(!$defined(destination)) {
            ...
            if(!destination) {
                throw ValueError(_('destination= is required for options like ' + optionString));//TODO change this
            }
	...
	_handleConflictError: function(action, conflictingActions) {
        ...
        throw ArgumentError(action, _('conflicting option string(s)') + ' : ' + conflictString);
	},
	_handleConflictResolve: function(action, conflictingActions) {
        ...
        conflictingActions.forEach(function(tuple) {
            ...
            action.optionStrings.remove(optionString);//TODO: change this
            this._optionStringActions.pop(optionString, None);//TODO: change this
...
var _MutuallyExclusiveGroup = _ArgumentGroup.extend('_MutuallyExclusiveGroup', {
	...
	_addAction: function(action) {
		if(action.required) {
            throw _('mutually exclusive arguments must be optional');//ValueError
		}
...
var ArgumentParser = _ActionContainer.extend('ArgumentParser', {
	...
	parseArgsKnown: function(args, namespace) {
        ...
        this._actions.forEach(function(action) {
            if(action.destination != SUPPRESS 
                ...
                if(isinstance(action.defaultValue, _basestring)) {//TODO: change this
        ...
        try {
            ...
            var error = _sys.exc_info()[1]; //TODO change this
            this.error(error);
        }
	},
	_parseArgsKnown: function(argStrings, namespace) {
	        ...
	        this._mutuallyExclusiveGroups.forEach(function(mutexGroup) {
	            ...
	            for i, mutexAction in enumerate(mutexGroup._groupActions):
	                conflicts = actionConflicts.setdefault(mutexAction, [])
	                conflicts.extend(groupActions[:i])
	                conflicts.extend(groupActions[i + 1:])
	        });
	        ...
	        argString_pattern_parts = []
	        argStrings_iter = iter(argStrings)
	        for i, argString in enumerate(argStrings_iter):

	            //all args after -- are non-options
	            if(argString == '--') {
	                argString_pattern_parts.append('-')
	                for argString in argStrings_iter:
	                    argString_pattern_parts.append('A')

	            //otherwise, add the arg to the arg strings
	            //and note the index if it was an option
	            } else {
	                option_tuple = this._parse_optional(argString)
	                if option_tuple is None:
	                    pattern = 'A'
	                else:
	                    option_string_indices[i] = option_tuple
	                    pattern = 'O'
	                argString_pattern_parts.append(pattern)
	            }
	        //join the pieces together to form the pattern
	        argStrings_pattern = ''.join(argString_pattern_parts)
	        ...
	        def take_action(action, argument_strings, option_string=None):
	            ...
	            if argument_values is not action.default:
	                ...
	                for conflict_action in actionConflicts.get(action, []):
	                    if conflict_action in seen_non_default_actions:
	                        msg = _('not allowed with argument %s')
	                        action_name = _get_action_name(conflict_action)
	                        throw ArgumentError(action, msg % action_name)
	        ...
	        def consume_optional(start_index):
	            ...
	            while True:
	                ...
	                if action is None:
	                    extras.append(argStrings[start_index])
	                ...
	                if explicit_arg is not None:
	                    ...
	                    if arg_count == 0 and option_string[1] not in chars:
	                        ...
	                        for char in this.prefix_chars:
	                            ...
	                            throw ArgumentError(action, msg % explicit_arg)
	                ...
	                //if successful, exit the loop
	                    ...
	                    start = start_index + 1
	                    selected_patterns = argStrings_pattern[start:]
	                    arg_count = match_argument(action, selected_patterns)
	                    stop = start + arg_count
	                    args = argStrings[start:stop]
	                    action_tuples.append((action, args, option_string))
	                    break
	        ...
	        def consume_positionals(start_index):
	            ...
	            selected_pattern = argStrings_pattern[start_index:]
	            arg_counts = match_partial(positionals, selected_pattern)

	            //slice off the appropriate arg strings for each Positional
	            //and add the Positional and its args to the list
	            for action, arg_count in zip(positionals, arg_counts):
	                args = argStrings[start_index: start_index + arg_count]
	        ...
	        while start_index <= max_option_string_index:
	            ...
	            if start_index not in option_string_indices:
	                strings = argStrings[start_index:next_option_string_index]
	        ...
	        extras.extend(argStrings[stop_index:])
	...
	_readArgsFromFiles: function(argStrings) {
        ...
        argStrings.forEach(function(argString) {
            ...
            //replace arguments referencing files with the file content
                ...
                try:
                    argsFile = open(argString.substr(1))
                    try:
                        argStrings = []
                        for argLine in argsFile.read().splitlines():
                            for arg in this.convert_argLine_to_args(argLine):
                                argStrings.append(arg)
                        argStrings = this._readArgsFromFiles(argStrings);
                        $extend(argStringsNew, argStrings);
                    finally:
                        argsFile.close();
                except IOError:
                    err = _sys.exc_info()[1]
	...
	_matchArgument: function(action, regexpArgStrings) {
        ...
        var matches = regexpArgStrings.search(regexpNargs);
	...
	_matchArgumentsPartial: functin(actions, regexpArgStrings) {
        //progressively shorten the actions list by slicing off the
        //final actions until we find a match
        result = [];
        for i in range(actions.length, 0, -1):
            actionSlice = actions[:i]
            pattern = ''.join([this._get_regexpNargs(action)
                               for action in actionSlice])
            match = _re.match(pattern, regexpArgStrings)
	...
	_parseOptional: function(argString) {
        ...
        if not argString[0] in this.prefixChars:
            return None

        //if the option string is present in the parser, return the action
        if argString in this._option_string_actions:
            action = this._option_string_actions[argString]
            return action, argString, None
        ...
        if '=' in argString:
            option_string, explicit_arg = argString.split('=', 1)
        ...
        option_tuples = this._get_option_tuples(argString)

        //if multiple actions match, the option string was ambiguous
        if len(option_tuples) > 1:
            options = ', '.join([option_string
                for action, option_string, explicit_arg in option_tuples])
            tup = argString, options
        ...
        if this._negative_number_matcher.match(argString):
	 ...
	 _getOptionTuples: function(optionString) {
	        ...
	        if(chars.indexOf(optionString[0]) >= 0 && chars.indexOf(optionString[1]) >= 0) {
	            ...
	            this._optionStringActions.forEach(function(optionString) {
	            	if(optionString.startswith(optionPrefix)) {//TODO: startswith
	        ...
	        //separate
	            ...
	            this._optionStringActions.forEach(function(optionString) {
	            	...
	            	} else if(optionString.startswith(optionPrefix)) {//TODO startswith
	                    result.push([action, optionString, argExplicit]);
	            	}
	            });
	    ...
	    _getValues: function(action, argStrings) {
	        ...
	        if([PARSER, REMAINDER].indexOf(action.nargs) < 0) {
	        	argStrings = $arrayCollect(argStrings, function(arrayElement) {
	        		return arrayElement != '--';
	        	});
	        }
	        ...
	        if(!argStrings && action.nargs == OPTIONAL) {
	            ...
	            if(isinstance(value, _basestring)) {//TODO change this
	        ...
	        //args, use the default if it is anything other than None
	            ...
	            value = $defined(action.defaultValue) ? action.defaultValue : argStrings;
	            this._checkValue(action, value);
	        ...
	        //REMAINDER arguments convert all values, checking none
	        	...
	        	value = [];
	        	argStrings.forEach(function(v) {
	        		value.push(this._getValue(action, v));
	        	});
	  ...
	  _getValue: function(action, argString) {
        ...
        if( $isCallable(typeFunc)) {
            throw ArgumentError(action, typeFunc + _(' is not callable'));
        }
        ...
        //TypeErrors or ValueErrors also indicate errors
            ...
            name = getattr(action.type, '__name__', repr(action.type))
            message = _('invalid %s value: %r')
            throw ArgumentError(action, message % (name, argString))
