...
(function(){
	...
	Class.create = function(className, proto) {
		...
		k.prototype.toRepr = function() {
			var className = $value(this.constructor.__class__, 'Object');
			var properties = [];
				...
				case 'object':
					if(value instanceof Array) {
						...
						value.forEach(function(v) {
							valueString.push(k.prototype.toRepr.bind(v)());
						});
						value = '[' + valueString.join(', ') + ']';
					} else {
						value = k.prototype.toRepr.bind(value)();
					}
...
var HelpFormatter = Class.create('HelpFormatter', {
	...
	_addItem: function(callback, args) {
	    this.sectionCurrent.addItem(callback, args);
	    return this;
	},
	...
	_HelpSection: Class.create('_HelpSection', {
		...
		addItem: function(callback, args) {
			args = args || [];
			this.items.push([callback, args]);
			return this;
		},
	    formatHelp: function() {
	        ...
	        var itemHelp = this.items.map(function(item) {
	        	var callback = item[0];
	        	var args = item[1];
	        	return callback.apply(context, args);//TODO apply context
	        });
...
var _ActionContainer = Class.create('_ActionContainer', {
	...
	_handleConflictError: function(action, conflictingActions) {
        ...
        throw new ArgumentError(action, _('conflicting option string(s)') + ' : ' + conflictString);
	},
...
var ArgumentParser = _ActionContainer.extend('ArgumentParser', {
	...
	addSubparsers: function(kwargs) {
        ...
        if($defined(kwargs['program'])) {
            ...
            positionals = this._getPositionalActions();
            groups = this._mutuallyExclusiveGroups;
            formatter.addUsage(this.usage, positionals, groups, '');
            kwargs['program'] = formatter.formatHelp().strip();//TODO: strip
        }
	            ...
	            if(argValues != action.defaultValue) {
	                ...
	                $value(actionConflicts[action], []).forEach(function(actionConflict) {
	                    if(seen_non_default_actions.indexOf(actionConflict) >= 0) {
	                        throw new ArgumentError(action, _('not allowed with argument ') + _getActionName(actionConflict));
	                    }
	                });
	            }
	                    ...
	                    if(argCount == 0 && this.prefixChars.indexOf(optionString[1]) < 0) {
	                        ...
	                        this.prefixChars.forEach(function(prefixChar) {
	                            ...
	                            if($defined(this._optionStringActions[optionString])) {
		                            ...
		                            throw new ArgumentError(action, _('ignored explicit argument ') + argExplicit);
	                            }
	                        });
	...
	_parseOptional: function(argString) {
	 ...
	 _getOptionTuples: function(optionString) {
	  ...
	  _getValue: function(action, argString) {
        ...
        if( $isCallable(typeFunc)) {
            throw new ArgumentError(action, typeFunc + _(' is not callable'));
        }
        ...
        //ArgumentTypeErrors indicate errors
	        ...
	        //catch (TypeError, ValueError):
	            name = action.type;
	            throw new ArgumentError(action, _('invalid ' + name + ' value: ' + argString));
	  	}
        //return the converted value
        return result;
	},
...
ArgumentError.prototype.initialize = function(argument, message) {
	this.argumentName = _getActionName(argument);
