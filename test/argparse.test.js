		...
		.addBatch(
				{
					...
					'getName() ' : {
						...
						'should return formatted optionStrings if set.' : function(
								topic) {
							assert.equal(topic.getName(), '-f/--foo');
						},
						'should return metavar if optionStrings is not set.' : function(
								topic) {
							topic.optionStrings = [];
							assert.equal(topic.getName(), 'BAR');
						},
						'should return metavar if optionStrings/metavar is not set.' : function(
								topic) {
							topic.optionStrings = [];
...
exports.ActionAppendTest = vows
		.describe('ActionAppend class')
		.addBatch(
				{
...
exports.ActionAppendConstantTest = vows
		.describe('ActionAppendConstant class')
		.addBatch(
				{
					'call(parser, namespace, values)' : {
						topic : function(item) {// Topic
...
exports.ActionCountTest = vows
		.describe('ActionCount class')
		.addBatch(
				{
					'call(parser, namespace, values)' : {
						topic : function(item) {// Topic
 ...
 * Action Test class
 */
exports.ActionStoreConstantTest = vows
		.describe('ActionStoreConstant class')
		.addBatch(
				{
					'call(parser, namespace, values)' : {
						topic : function(item) {// Topic
						...
						'should store constant into namespace (ignoring values)' : function(
								topic) {
...
exports.ActionStoreFalseTest = vows
		.describe('ActionStoreFalse class')
		.addBatch(
				{
					'call(parser, namespace, values)' : {
						topic : function(item) {// Topic
		...
		.addBatch(
				{
					"new Namespace({options}) " : {
						...
						'should initialize the attributes transparently' : function(
								topic) {// Vow
 ...
 */
exports.ArgumentParserTest = vows
		...
		.addBatch( { // Batch are executed sequentially
					"new ArgumentParser({}) " : {
						...
						'should set the default help formatter' : function(
								topic) {// Vow
					...
					"_getFormatter() " : {
						...
						"should return <HelpFormatter> object for default configuration" : function(
								topic) {
							var formatter = topic._getFormatter();
							assert
									.ok(formatter instanceof argparse.HelpFormatter);
							// check that the program is set to foo
							assert.equal(formatter.program, 'foo');
						},
						"should return <other classes...> object if set in configuration" : function(
								topic) {
							[ 'HelpFormatterArgumentDefaults',
									...
									.forEach(function(className) {
										...
										var formatter = topic._getFormatter();
										assert
												.ok(formatter instanceof argparse[className]);
									});
					...
					'_printMessage()' : {
						...
						"should print message into file" : function(topic) {
							...
							assert.equal(buffer.toString('utf8', 0,
					...
					'formatUsage()' : {
						...
						'should return "usage: %program%" without help' : function(
								topic) {
							assert.equal(topic.formatUsage(), 'usage: foo\n');
						},
						'should return "usage: %program% help" with help' : function(
								topic) {
							topic.addArgument( [ '-h', '--help' ], {
								action : 'help',
								help : 'foo bar'
							});
							assert.equal(topic.formatUsage(),
									'usage: foo [-h]\n');
						}
					},
					'formatHelp()' : {
					...
					'parseArgs() / with default argument' : {
						...
						'should parse short syntax [-f, baz] to {foo:bar}' : function(
								topic) {
							assert.deepEqual(topic.parseArgs( [ '-f', 'baz' ]),
									Namespace( {
										foo : 'baz'
									}));
						},
						'should parse short explicit syntax [-f=baz] to {foo:bar}' : function(
								topic) {
							assert.deepEqual(topic.parseArgs( [ '-f=baz' ]),
									Namespace( {
										foo : 'baz'
							...
							assert.deepEqual(topic
									.parseArgs( [ '-f=baz=notparsed' ]),
									Namespace( {
										foo : 'baz'
									}));
						},
						'should parse long syntax [--foo baz] to {foo:baz}' : function(
								topic) {
							assert.deepEqual(topic
									.parseArgs( [ '--foo', 'baz' ]),
									Namespace( {
						...
						'should parse long explicit syntax [--foo=baz] to {foo:baz}' : function(
								topic) {
							assert.deepEqual(topic.parseArgs( [ '--foo=baz' ]),
									Namespace( {
										foo : 'baz'
									}));
							assert.deepEqual(topic
									.parseArgs( [ '--foo=baz=notparsed' ]),
									Namespace( {
										foo : 'baz'
									}));
						},
						'should parse [--foo] to {foo:defaultVal}' : function(
								topic) {
							assert.deepEqual(topic.parseArgs( [ '--foo' ]),
									Namespace( {
										foo : 'defaultVal'
									}));
						},
					...
					'parseArgs() / with required argument' : {
						...
						'should parse [--foo] throwing an error' : function(
								topic) {

							assert.throws(function() {
								topic.parseArgs( [ '--foo' ]);// TODO find way
									// to check the
									// error type
								});
						}
